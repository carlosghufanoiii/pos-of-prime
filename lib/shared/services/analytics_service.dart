...
/// Calculates metrics from live data in Appwrite
class AnalyticsService {
  ...
  static Future<Map<String, dynamic>> getDashboardStats() async {
    try {
      ...
      final bartenderUsers = users.where((u) => u.role.name == 'bartender').length;
  ...
  static Future<Map<String, dynamic>> getSalesAnalytics() async {
    try {
      ...
      final monthlySales = _calculateSalesForPeriod(paidOrders, monthStart, now);
      
      // Calculate yearly sales
      final yearSales = _calculateSalesForPeriod(paidOrders, yearStart, now);

      // Payment method breakdown
      final cashSales = _calculateSalesByPaymentMethod(paidOrders, PaymentMethod.cash);
      final cardSales = _calculateSalesByPaymentMethod(paidOrders, PaymentMethod.card);
      final eWalletSales = _calculateSalesByPaymentMethod(paidOrders, PaymentMethod.eWallet);
  ...
  static Future<Map<String, dynamic>> getSystemStats() async {
    try {
      ...
      final pendingOrders = orders.where((o) => o.status == OrderStatus.pendingApproval).length;
      final approvedOrders = orders.where((o) => o.status == OrderStatus.approved).length;
      final inPrepOrders = orders.where((o) => o.status == OrderStatus.inPrep).length;
      final readyOrders = orders.where((o) => o.status == OrderStatus.ready).length;
      final servedOrders = orders.where((o) => o.status == OrderStatus.served).length;
    ...
    try {
      ...
      final recentOrders = orders
          ...
          .map((order) => {
                ...
                'description': 'Order ${order.id} ${_getOrderAction(order.status)}',
  ...
  static double _calculateSalesForPeriod(List<Order> orders, DateTime start, DateTime end) {
    return orders
        .where((order) => 
  ...
  static double _calculateSalesByPaymentMethod(List<Order> orders, PaymentMethod method) {
    return orders
  ...
  static double _calculateAverageProcessingTime(List<Order> orders) {
    final processedOrders = orders.where((o) => 
